...
const AdminSchoolEnrollments = () => {
    ...
    const loadEnrollments = async () => {
        try {
            ...
            const records = await pb.collection('school_enrollments').getList(1, 100, {
                filter: filterQuery,
                sort: '-created',
            });
            ...
            console.error('Failed to load school enrollments:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            await pb.collection('school_enrollments').update(id, {
                status: newStatus,
            });
            loadEnrollments();
        } catch (error) {
    ...
    const deleteEnrollment = async (id) => {
        if (!confirm('Are you sure you want to delete this school enrollment?')) {
            return;
        }

        try {
            await pb.collection('school_enrollments').delete(id);
            loadEnrollments();
        } catch (error) {
    ...
    const filteredEnrollments = enrollments.filter(enrollment =>
        ...
        enrollment.schoolName.toLowerCase().includes(searchTerm.toLowerCase())
    );
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <h1 className="text-3xl font-bold text-gray-900">School Enrollments</h1>
                ...
                <div className="space-y-4">
                    {filteredEnrollments.length === 0 ? (
                        <div className="bg-white rounded-lg shadow-md p-12 text-center">
                            <p className="text-gray-500 text-lg">No school enrollments found</p>
                        </div>
                    ) : (
                            ...
                            >
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-lg text-[#FF6B35] font-semibold mt-1">{enrollment.schoolName}</p>
                                ...
                                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <p className="text-sm font-semibold text-gray-700">School Address:</p>
                                            <p className="text-gray-600">{enrollment.schoolAddress}</p>
                                        </div>
                                ...
                                <div className="flex gap-2">
                                    <a
                                        href={`mailto:${enrollment.email}?subject=Re: Language Training Partnership for ${enrollment.schoolName}`}
...
export default AdminSchoolEnrollments;
